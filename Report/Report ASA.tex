...
Only change to utilities are it does not choose other agent target.

Explain how the exchange works, when it starts, when the other agent ignores, the fact that only one agent calculates the targets and then each agent calculates the plan. Also utility for exchange specifically for second map of second challenge.

Same thing for the block, this time only one agent checks if it is possible with the shared same information.

Everything else is the same except with shared information.
